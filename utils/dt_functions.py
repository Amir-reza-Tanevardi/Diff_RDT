...
def load_minari_trajectories(
    ...
    if config.sample_ratio < 1.0:
        ...
        dataset_path = "/work/pi_aghasemi_umass_edu/afzali_umass/CM_pooria/Diff_RDT/utils/your_path_of_dataset/original/mujoco/walker2d/medium-v0_ratio_0.1.pt"
    ...
    if config.corruption_mode != "none":
        dataset_dict, attack_indexs = attack_dataset(config, dataset_dict, logger)
        attack_mask[attack_indexs] = 1
    dataset_dict["attack_mask"] = attack_mask

    state_mean, state_std = 0.0, 1.0
    if config.normalize:
        state_mean = dataset_dict["observations"].mean(0, keepdims=True)
        state_std = dataset_dict["observations"].std(0, keepdims=True) + 1e-6

    traj, traj_len = [], []
    data_ = defaultdict(list)
    for i in trange(dataset_dict["rewards"].shape[0], desc="Processing trajectories"):
        data_["observations"].append(dataset_dict["observations"][i])
        data_["actions"].append(dataset_dict["actions"][i])
        data_["rewards"].append(dataset_dict["rewards"][i])
        data_["attack_mask"].append(dataset_dict["attack_mask"][i])
...
class SequenceDataset:
    def __init__(self, config, logger: Logger = None):
        # Load Minari dataset
        self.dataset, info = load_minari_trajectories(config, config.env, gamma=1.0, logger=logger)
        ...
        # Assuming MinariDataset exposes similar stats
        self.state_mean = info['obs_mean']
        self.state_std = info['obs_std']
        
        # For sampling, we assume the dataset provides episode lengths or a similar metric
        self.sample_prob = self._compute_sample_probabilities()

        self.float_dtype = np.float32

    def _compute_sample_probabilities(self):
        # Assuming you can calculate probabilities based on episode lengths or a similar metric
        episode_lengths = np.array([len(episode['observations']) for episode in self.dataset])
        return episode_lengths / episode_lengths.sum()

    def correct(self, traj_indexs, time_steps, correct_data, correct_type):
        for i, (traj_i, step_j) in enumerate(zip(traj_indexs, time_steps)):
            if step_j < len(self.dataset[traj_i][correct_type]):
                self.dataset[traj_i][correct_type][step_j] = correct_data[i]

    def __prepare_sample(self, traj_idx, start_idx):
        traj = self.dataset[traj_idx]
        
        # Minari episodes are accessed similarly to how you did with D4RL
        states = traj['observations'][start_idx:start_idx + self.seq_len]
        actions = traj['actions'][start_idx:start_idx + self.seq_len]
        
        if self.recalculate_return:
            returns = self.discounted_cumsum(traj['rewards'][start_idx:], gamma=1.0)[:self.seq_len].reshape(-1, 1)
        else:
        ...
        if states.shape[0] < self.seq_len:
            states = self.pad_along_axis(states, self.seq_len)
            actions = self.pad_along_axis(actions, self.seq_len)
            returns = self.pad_along_axis(returns, self.seq_len)
            rewards = self.pad_along_axis(rewards, self.seq_len)
            attack_mask = self.pad_along_axis(attack_mask, self.seq_len)
    ...
    def discounted_cumsum(self, rewards, gamma):
        return np.flip(np.cumsum(np.flip(rewards, axis=0) * gamma, axis=0), axis=0)

    def pad_along_axis(self, array, pad_to):
        return np.pad(array, ((0, pad_to - len(array)), (0, 0)), mode='constant', constant_values=0)
    ...
    def get_batch(self, batch_size: int) -> Tuple[torch.Tensor, ...]:
        ...
        for traj_id in traj_ids:
            start_idx = np.random.randint(0, len(self.dataset[traj_id]["rewards"]))
